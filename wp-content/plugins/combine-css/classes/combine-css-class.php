...
class CombineCSS {
	...
	*/
	function init() {
		...
		elseif ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {}
			...
			// gather and combine css function calls
	...
	*/
	function combine_css () {
		...
		if ( file_exists ( $this->css_path ) ) {
			echo "\t\t<link rel='stylesheet' id='combine-css' href='" . str_replace( get_option( 'siteurl' ), $this->css_domain, $this->get_plugin_url() . 'css.php?token=' . $this->css_token . '&#038;ver=' . self::version ) . "' type='text/css' media='all' />\n";
		}
		return $to_do;
	}
	...
	*/
	function add_settings_page() {
		if ( current_user_can( 'manage_options' ) ) add_options_page( self::pname, self::pname, 'manage_options', 'combine-css-settings', array( &$this, 'settings_page' ) );
	}
	...
	*/
	function settings_page() {
		if ( isset( $_POST['combine-css_update_settings'] ) ) $this->update_settings();
	...
	function delete_cache_button() {
		...
		$wp_admin_bar->add_menu(
								array(
										...
										'meta' => array( 'title' => __( 'Delete CSS Cache', self::nspace ) ),
